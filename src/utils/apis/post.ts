...
import type { Post, PaginatedPostResponse, Comment, LikeUser } from '@/utils/types/post';

export const fetchRecommendedPosts = async (page = 1, limit = 5): Promise<PaginatedPostResponse> => {
    const res = await axiosWithConfig.get(`/posts/recommended?page=${page}&limit=${limit}`);
    return res.data;
};
...
export const searchPosts = async (query: string): Promise<PaginatedPostResponse> => {
    const res = await axiosWithConfig.get(`/posts/search?q=${encodeURIComponent(query)}`);
    return res.data;
};
...
export const createPost = async (data: Partial<Post>): Promise<Post> => {
    const res = await axiosWithConfig.post('/posts', data);
    return res.data;
};
